export default {
    ...
    activityBubbleName(prettyValue, rawValue, itemPath, item, definitions) {
        ...
        const locationRelease = item.locationReleases && item.locationReleases[locationReleaseIndex];

        const destination = locationRelease && definitions.DestinyDestinationDefinition[locationRelease.destinationHash];

        const bubble = destination && destination.bubbles.find(bub => (bub.hash = rawValue));
    ...
    locationHash(prettyValue, rawValue, itemPath, item, definitions) {
        ...
        const showActivityString = activityString && (!destinationString || destination.displayProperties.name != activity.displayProperties.name);

        const strings = [vendorString, showActivityString && activityString, bubbleString, destinationString].filter(Boolean);
